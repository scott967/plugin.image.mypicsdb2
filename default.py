...
class Main:
                    ...
                    if dialog.yesno(common.getstring(30000), common.getstring(30206)):
                        if newroot.startswith('multipath://'):
                            ...
                            for item in newpartialroot:
                                ...
                                common.run_script("%s,%s --rootpath=%s" % ("script.module.mypicsdb2scan",
                                                  recursive and "-r, " or "", common.quote_param(unquote(item))))
                            ...
                            common.run_script("%s,%s --rootpath=%s" % ("script.module.mypicsdb2scan",
                                              recursive and "-r, " or "", common.quote_param(newroot)))
    ...
    def show_pics(self):
        ...
        # we are showing pictures for a FOLDER selection
            ...
            filelist = [row for row in MPDB.cur.request("""SELECT p.FullPath,f.strFilename FROM Files f, Folders p WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND f.idFolder=p.idFolder AND p.ParentFolder in ('%s') ORDER BY ImageDateTime ASC LIMIT %s OFFSET %s""" % (
