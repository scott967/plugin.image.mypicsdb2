...
class Main:
    def __init__(self):
        ...
        try:
            ...
            common.log("Main.add_directory",  f"{Exception} - {str(msg)}", xbmc.LOGERROR)
            raise msg
    ...
    def add_directory(self, name, params, action, iconimage, fanart=None, contextmenu=None, total=0, info="*", replacemenu=True):
        ...
        try:
            ...
            u = sys.argv[0]+"?"+parameter+"&action=" + \
                repr(str(action))+"&name="+repr(common.quote_param(name))
    ...
    def add_action(self, name, params, action, iconimage, fanart=None, contextmenu=None, total=0, info="*", replacemenu=True):
        ...
        try:
            try:
                ...
                common.log("Main.add_action",  f"{Exception} - {str(msg)}", xbmc.LOGERROR)
                parameter = ""
    ...
    def show_date(self):
        ...
        dptd = dptd.replace("%b", monthname[strptime(
            self.args.value, thisdateformat).tm_mon - 1])
        dptd = dptd.replace("%B", fullmonthname[strptime(
        ...
        if count > 0:
            self.add_directory(name=common.getstring(30100) % (nameperiode, count),  # libellé#"All the period %s (%s pics)"%(self.args.value,MPDB.count_pics_in _period(allperiod,self.args.value)), #libellé
                               params=[("method", "date"), ("period", allperiod), (
                                   # paramètres
                                   "value", self.args.value), ("page", ""), ("viewmode", "view")],
        ...
        if count > 0 and self.args.period == "year":
            self.add_directory(name=common.getstring(30054) % (count),
                               ...
                               iconimage=join(
                                   PIC_PATH, "folder_date.png"),  # icone
                               contextmenu=[(common.getstring(30152), f"RunPlugin(\"{sys.argv[0]}?action='addfolder'&method='date'&period='{allperiod}'&value='{self.args.value}'&viewmode='scan'\")"),]
                               )
        ...
        for period in listperiod:
            if period:
                if action == "showpics":
                    context = [(common.getstring(30152), f"RunPlugin(\"{sys.argv[0]}?action='addfolder'&method='date'&period='{nextperiod}'&value='{period}'&page=''&viewmode='scan'\")")]
                else:
                    context = [(common.getstring(30152), f"RunPlugin(\"{sys.argv[0]}?action='addfolder'&method='date'&period='{self.args.period}'&value='{period}'&viewmode='scan'\")")]
    ...
    def show_folders(self):
        ...
        for idchildren, childrenfolder in childrenfolders:
            common.log("Main.show_folders", f"children folder = {childrenfolder}")
            ...
            if count > 0:
                self.add_directory(name=f"{childrenfolder} ({count} {common.getstring(30050)})",  # libellé
                                   ...
                                   contextmenu=[(common.getstring(30212), f"Container.Update(\"{sys.argv[0]}?action='rootfolders'&do='addrootfolder'&addpath='{common.quote_param(path)}'&exclude='1'&viewmode='view'\",)"),],
                                   # nb total d'éléments
        ...
        if min_rating > 0:
            picsfromfolder = [row for row in MPDB.cur.request_with_binds(
                "SELECT p.FullPath, f.strFilename FROM Files f, Folders p WHERE f.idFolder=p.idFolder AND f.idFolder=? AND f.ImageRating > ? order by f.imagedatetime", (self.args.folderid, min_rating, ))]
        else:
            picsfromfolder = [row for row in MPDB.cur.request_with_binds(
                "SELECT p.FullPath, f.strFilename FROM Files f, Folders p WHERE f.idFolder=p.idFolder AND f.idFolder=? order by f.imagedatetime", (self.args.folderid, ))]
        ...
        for path, filename in picsfromfolder:
            ...
            self.add_picture(filename, path, count=count, contextmenu=context,
                             fanart=xbmcplugin.getSetting(
                                 int(sys.argv[1]), 'usepicasfanart') == 'true' and join(path, filename)
                             )
    ...
    def show_translationeditor(self):
        ui = translationeditor.TranslationEditor(
            "script-mypicsdb-translationeditor.xml", common.getaddon_path(), "Default")
        ui.doModal()
        del ui
    ...
    def show_wizard(self):
        if self.args.wizard == 'dialog':
            ...
            if len(GlobalFilterTrue) > 0 or len(GlobalFilterFalse) > 0 or start_date != '' or end_date != '':
                xbmc.executebuiltin("Container.Update(%s?action='showpics'&viewmode='view'&method='wizard'&matchall='%s'&kw='%s'&nkw='%s'&start='%s'&end='%s')" % (
            ...
            for filtername in filterlist:
                ...
                self.add_directory(name="%s" % (filtername),
                                   ...
                                   contextmenu=[(common.getstring(
                                       30152), "RunPlugin(\"%s?action='addfolder'&method='wizard_settings'&filtername='%s'&viewmode='scan'\")" % (sys.argv[0], filtername)),],
                                   # contextmenu   = [('','')],
                                   total=total)
            xbmcplugin.addSortMethod(
                int(sys.argv[1]), xbmcplugin.SORT_METHOD_LABEL)
        ...
        for tag, nb in listtags:
            if nb:
                self.add_directory(name="%s (%s %s)" % (tag, nb, common.getstring(30052)),  # libellé
                                   params=[("method", "tagtype"), ("tagtype", tag),
                                           # paramètres
        ...
        for tag, nb in listtags:
            if nb:
                self.add_directory(name="%s (%s %s)" % (tag, nb, common.getstring(30050)),  # libellé
                                   params=[("method", "tag"), ("tag", tag), ("tagtype", tagtype),
                                           # paramètres
                                           ("page", "1"), ("viewmode", "view")],
                                   action="showpics",  # action
                                   iconimage=join(
                                   ...
                                   contextmenu=[(common.getstring(30152), "RunPlugin(\"%s?action='addfolder'&method='tag'&tag='%s'&tagtype='%s'&viewmode='scan'\")" % (sys.argv[0], common.quote_param(tag), tagtype)),
                                                (common.getstring(30061), "RunPlugin(\"%s?action='showpics'&method='tag'&page=''&viewmode='zip'&name='%s'&tag='%s'&tagtype='%s'\")" % (
                                                    sys.argv[0], common.quote_param(tag), common.quote_param(tag), tagtype)),
                                                (common.getstring(30062), "RunPlugin(\"%s?action='showpics'&method='tag'&page=''&viewmode='export'&name='%s'&tag='%s'&tagtype='%s'\")" % (
                                                    sys.argv[0], common.quote_param(tag), common.quote_param(tag), tagtype))
                                                ],  # menucontextuel
                                   total=total)  # nb total d'éléments
        xbmcplugin.addSortMethod(
    ...
    def show_period(self):  # TODO finished the datestart and dateend editing
        ...
        if self.args.period == "setperiod":
            ...
            nameddates = [strftime(self.prettydate(common.getstring(30002), strptime(
                date, "%Y-%m-%d")), strptime(date, "%Y-%m-%d")) for date in dateofpics]
            common.log("show_period >> namedates", nameddates)

            if len(nameddates):

                dialog = xbmcgui.Dialog()
                rets = dialog.select(common.getstring(30107), ["[[%s]]" % common.getstring(
                    30114)] + nameddates)  # dateofpics)#choose the start date
                if not rets == -1:  # is not canceled
                    if rets == 0:  # input manually the date
                        d = dialog.numeric(1, common.getstring(30117), strftime(
                            "%d/%m/%Y", strptime(dateofpics[0], "%Y-%m-%d")))
                    ...
                    if datestart != '':
                        ...
                        retf = dialog.select(common.getstring(30108), [
                                             "[[%s]]" % common.getstring(30114)] + nameddates[deb:])
                        if not retf == -1:  # if end date is not canceled...
                            if retf == 0:  # choix d'un date de fin manuelle ou choix précédent de la date de début manuelle
                                d = dialog.numeric(1, common.getstring(30118), strftime(
                                    "%d/%m/%Y", strptime(dateofpics[-1], "%Y-%m-%d")))
                                if d != '':
                                    dateend = strftime(
                                        "%Y-%m-%d", strptime(d.replace(" ", "0"), "%d/%m/%Y"))
                                else:
                            ...
                            if dateend != '':
                                ...
                                #
                                kb = xbmc.Keyboard(common.smart_utf8(common.getstring(30109) % (
                                    datestart, dateend)), common.getstring(30110), False)
                                kb.doModal()
                                if (kb.isConfirmed()):
                                ...
                                MPDB.period_add(common.smart_unicode(titreperiode), common.smart_unicode(
                                    datestart), common.smart_unicode(dateend))
                update = True
            else:
        ...
        for periodname, dbdatestart, dbdateend in MPDB.periods_list():
            ...
            self.add_directory(name="%s [COLOR=C0C0C0C0](%s)[/COLOR]" % (periodname,
                                                                         common.getstring(30113) % (strftime(self.prettydate(common.getstring(30002), strptime(datestart, "%Y-%m-%d")), strptime(datestart, "%Y-%m-%d")),
                                                                                                    strftime(self.prettydate(common.getstring(30002), strptime(
                                                                                                        dateend, "%Y-%m-%d")), strptime(dateend, "%Y-%m-%d"))
                                                                                                    )),  # libellé
                               ...
                               contextmenu=[(common.getstring(30111), "RunPlugin(\"%s?action='removeperiod'&viewmode='view'&periodname='%s'&period='period'\")" % (sys.argv[0], common.quote_param(periodname))),
                                            (common.getstring(30112), "RunPlugin(\"%s?action='renameperiod'&viewmode='view'&periodname='%s'&period='period'\")" % (
                                                sys.argv[0], common.quote_param(periodname))),
                                            (common.getstring(30152), "RunPlugin(\"%s?action='addfolder'&method='date'&period='period'&datestart='%s'&dateend='%s'&viewmode='scan'\")" % (
    ...
    def show_collection(self):
            ...
            if ret > -1:
                ...
                if (kb.isConfirmed()):
                    ...
                    if rows != None:
                        ...
                        common.log("show_collection", str(
                            filters[ret]) + " is empty and therefore not created.", xbmc.LOGINFO)
            refresh = True
        ...
        self.add_directory(name=common.getstring(30160),
                           ...
                           iconimage=join(
                               PIC_PATH, "folder_collections.png"),  # icone
                           contextmenu=None)  # menucontextuel
        ...
        for collection in MPDB.collections_list():
            self.add_action(name=collection[0],
                            params=[("method", "collection"), ("collect", collection[0]),
                                    # paramètres
                                    ("page", "1"), ("viewmode", "slideshow")],
                            action="showpics",  # action
                            iconimage=join(
                                PIC_PATH, "folder_collections.png"),  # icone

                            contextmenu=[
                (common.getstring(30169), "Container.Update(\"%s?action='showpics'&method='collection'&page=''&viewmode='view'&name='%s'&collect='%s'\")" % (
                    sys.argv[0], common.quote_param(collection[0]), common.quote_param(collection[0]))),
                (common.getstring(30149), "RunPlugin(\"%s?action='collectionaddplaylist'&viewmode='view'&collect='%s'\")" % (
                    sys.argv[0], common.quote_param(collection[0]))),
                (common.getstring(30158), "RunPlugin(\"%s?action='removecollection'&viewmode='view'&collect='%s'\")" % (
                    sys.argv[0], common.quote_param(collection[0]))),
                (common.getstring(30159), "RunPlugin(\"%s?action='renamecollection'&viewmode='view'&collect='%s'\")" % (
                    sys.argv[0], common.quote_param(collection[0]))),
                (common.getstring(30061), "RunPlugin(\"%s?action='showpics'&method='collection'&page=''&viewmode='zip'&name='%s'&collect='%s'\")" % (
                    sys.argv[0], common.quote_param(collection[0]), common.quote_param(collection[0]))),
                (common.getstring(30062), "RunPlugin(\"%s?action='showpics'&method='collection'&page=''&viewmode='export'&name='%s'&collect='%s'\")" % (
                    sys.argv[0], common.quote_param(collection[0]), common.quote_param(collection[0])))
            ])  # menucontextuel
        ...
        for tag, _ in listtags:
            ...
            if compte:
                ...
                self.add_directory(name=common.getstring(30116) % (compte, motrecherche, tag),  # files_fields_description.has_key(colname) and files_fields_description[colname] or colname),
                                   params=[("method", "search"), ("field", u"%s" % common.smart_unicode(
                                       # paramètres
                                       tag)), ("searchterm", u"%s" % common.smart_unicode(motrecherche)), ("page", "1"), ("viewmode", "view")],
                                   action="showpics",  # action
                                   iconimage=join(
                                       PIC_PATH, "folder_search.png"),
                                   # menucontextuel
                                   contextmenu=[(common.getstring(30152), "RunPlugin(\"%s?action='addfolder'&method='search'&field='%s'&searchterm='%s'&viewmode='scan'\")" % (sys.argv[0], tag, motrecherche))])
        if not result:
            dialog = xbmcgui.Dialog()
    ...
    def show_roots(self):
        ...
        if self.args.do == "addroot" or self.args.do == "addpicturessource":  # add a root to scan
                    ...
                    try:
                        if source.startswith('multipath://'):
                            ...
                            for item in newpartialroot:
                                ...
                                MPDB.add_root_folder(
                                    unquote(item), True, True, 0)
                                common.log(
                        ...
                        xbmc.executebuiltin(
                            "Container.Refresh(\"%s?action='rootfolders'&do='showroots'&exclude='0'&viewmode='view'\",)" % (sys.argv[0],))
            ...
            if str(self.args.exclude) == "1":
                ...
                recursive = dialog.yesno(common.getstring(30000), common.getstring(
                    30202)) and 1 or 0  # browse recursively this folder ?
                ...
                try:
                    if newroot.startswith('multipath://'):
                        ...
                        for item in newpartialroot:
                            # TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                            MPDB.add_root_folder(
                                unquote(item), recursive, update, 0)
                            common.log(
                                "Main.show_roots", 'Multipath addroot for part "%s" done' % unquote(item))
                    else:
                        # TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                    ...
                    if dialog.yesno(common.getstring(30000), common.getstring(30206)):
                        if newroot.startswith('multipath://'):
                            ...
                            for item in newpartialroot:
                                ...
                                common.run_script("%s,%s --rootpath=%s" % (join(
                                    home, "scanpath.py"), recursive and "-r, " or "", common.quote_param(unquote(item))))
                            ...
                            common.run_script("%s,%s --rootpath=%s" % (join(
                                home, "scanpath.py"), recursive and "-r, " or "", common.quote_param(newroot)))
            ...
            try:
                ...
                if dialog.yesno(common.getstring(30250), common.smart_utf8(common.getstring(30251)) % common.smart_utf8(self.args.delpath)):
                    ...
                    if self.args.delpath != 'neverexistingpath':
                        common.show_notification(common.getstring(
                            30000), common.getstring(30205), 3000, join(home, "icon.png"))
            ...
            if common.getaddon_setting('scanning') == 'false':
                if str(self.args.exclude) == "0":
                    path, recursive, update, exclude = MPDB.get_root_folders(
                        self.args.rootpath)
                    common.run_script("%s,%s --rootpath=%s" % (join(home, "scanpath.py"),
                                      recursive and "-r, " or "", common.quote_param(path)))
            ...
            for path, recursive, update in includefolders:
                ...
                self.add_action(name="[COLOR=FF66CC00][B][ + ][/B][/COLOR] "+path+" [COLOR=FFC0C0C0][recursive="+srec+" , update="+supd+"][/COLOR]",
                                ...
                                contextmenu=[(common.getstring(30206), "Notification(TODO : scan folder,scan this folder now !,3000,%s)" % join(home, "icon.png")),
                                             (common.getstring(30207), "Container.Update(\"%s?action='rootfolders'&do='delroot'&delpath='%s'&exclude='1'&viewmode='view'\",)" % (
    ...
    def period_rename(self):
        ...
        datestart, dateend = MPDB.cur.request_with_binds(
            """SELECT DateStart,DateEnd FROM Periodes WHERE PeriodeName=? """, (periodname,))[0]
        common.log("", "datestart = %s" % datestart)
        common.log("", "dateend = %s" % dateend)
        dialog = xbmcgui.Dialog()
        d = dialog.numeric(1, "Input start date for period", strftime(
            "%d/%m/%Y", strptime(str(datestart), "%Y-%m-%d %H:%M:%S")))
        datestart = strftime(
            "%Y-%m-%d", strptime(d.replace(" ", "0"), "%d/%m/%Y"))

        d = dialog.numeric(1, "Input end date for period", strftime(
            "%d/%m/%Y", strptime(str(dateend), "%Y-%m-%d %H:%M:%S")))
        dateend = strftime(
            "%Y-%m-%d", strptime(d.replace(" ", "0"), "%d/%m/%Y"))
    ...
    def collection_add_pic(self):
        ...
        common.show_notification(common.getstring(30000), common.getstring(
            30154) + ' ' + namecollection, 3000, join(home, "icon.png"))
    ...
    def collection_add_folder(self):
        ...
        for path, filename in filelist:  # on les ajoute une par une
            ...
            MPDB.collection_add_pic(namecollection, path, filename)
        common.show_notification(common.getstring(30000), common.getstring(
            30161) % len(filelist)+' '+namecollection, 3000, join(home, "icon.png"))
    ...
    def collection_delete(self):
        ...
        if dialog.yesno(common.getstring(30150), common.getstring(30251) % self.args.collect):
            ...
            xbmc.executebuiltin(
                "Container.Update(\"%s?action='showcollection'&viewmode='view'&collect=''&method='show'\" , replace)" % sys.argv[0], )
    ...
    def collection_del_pic(self):
        ...
        xbmc.executebuiltin("Container.Update(\"%s?action='showpics'&viewmode='view'&page='1'&collect='%s'&method='collection'\" , replace)" % (
    ...
    def set_properties(self):
        ...
        Categories = MPDB.cur.request(
            """select count(distinct tf.idFile) from TagTypes tt, TagContents tc, TagsInFiles tf where tt.idTagType = tc.idTagType and tc.idTagContent = tf.idTagContent and tt.TagTranslation = ( select TagTranslation from TagTypes tti where tti.TagType = 'Category')""")[0]
    ...
    def set_slideshow(self):
        ...
        for _i in range(_current,  _limit + 1):
            ...
            _json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "Playlist.Add", "params": {"playlistid": 2, "item": {"file" : "%s"}}, "id": 1}' % (
    ...
    def show_pics(self):
        ...
        # we are showing pictures for a RANDOM selection
            ...
            try:
                count = [row for row in MPDB.cur.request(
                    """SELECT count(*) FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ?""", (min_rating,))][0][0]
            except:
                count = 0
            ...
            if MPDB.db_backend.lower() == 'mysql':
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath, strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? ORDER BY RAND() LIMIT %s OFFSET %s""" % (limit, offset), (min_rating,))]
            else:
                if count < limit:
        ...
        # we are showing pictures for a DATE selection
            ...
            formatstring = {"wo": "", "year": "%Y", "month": "%Y-%m",
                            "date": "%Y-%m-%d", "": "%Y", "period": "%Y-%m-%d"}[self.args.period]
            if self.args.period == "wo":
                ...
                filelist = MPDB.search_between_dates(DateStart=(self.args.datestart, formatstring), DateEnd=(
                    self.args.dateend, formatstring), MinRating=min_rating)
                ...
                amaxi = max(listyears)
                if amini and amaxi:
                    filelist = MPDB.search_between_dates(("%s" % (amini), formatstring), ("%s" % (
                        amaxi), formatstring), MinRating=min_rating)
                else:
                    filelist = []

        # we are showing pictures for a TAG selection
        elif self.args.method == "wizard":
            filelist = MPDB.filterwizard_result(
                self.args.kw, self.args.nkw, self.args.matchall, self.args.start, self.args.end, min_rating)
        ...
        # we are showing pictures for a FOLDER selection
            ...
            if MPDB.con.get_backend() == "mysql":
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND datetime(ImageDateTime) BETWEEN SysDate() - INTERVAL 1 MONTH AND SysDate() ORDER BY ImageDateTime ASC LIMIT %s OFFSET %s""" % (limit, offset), (min_rating,))]
            else:
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND datetime(ImageDateTime) BETWEEN datetime('now','-1 months') AND datetime('now') ORDER BY ImageDateTime ASC LIMIT %s OFFSET %s""" % (limit, offset), (min_rating,))]
            ...
            if MPDB.con.get_backend() == "mysql":
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND DateAdded>=SysDate() - INTERVAL %s DAY ORDER BY DateAdded ASC LIMIT %s OFFSET %s""" % (numberofdays, limit, offset), (min_rating,))]
            else:
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND DateAdded >= datetime('now','start of day','-%s days') ORDER BY DateAdded ASC LIMIT %s OFFSET %s""" % (numberofdays, limit, offset), (min_rating,))]
            ...
            filelist = [row for row in MPDB.cur.request(
                """SELECT strPath,strFilename FROM Files WHERE ftype="video" ORDER BY ImageDateTime DESC LIMIT %s OFFSET %s""" % (limit, offset))]
        ...
        if self.args.viewmode == "zip":
            ...
            if isfile(destination):
                dialog = xbmcgui.Dialog()
                ok = dialog.yesno(common.getstring(30000), common.getstring(30064) % basename(destination), dirname(
                    # Archive already exists, overwrite ?
                    destination), common.getstring(30065))
                if not ok:
                    # todo, ask for another name and if cancel, cancel the zip process as well
                    common.show_notification(common.getstring(
                        30000), common.getstring(30066), 3000, join(home, "icon.png"))
            ...
            tar = taropen(destination.encode(
                sys.getfilesystemencoding()), mode="w:gz")
            error = 0
            ...
            for (path, filename) in filelist:
                ...
                if common.smart_unicode(picture) == common.smart_unicode(destination):
                    continue
                # adding picture to the archive
                pDialog.update(int(100*(compte/float(len(filelist)))),
                               common.getstring(30067), picture)
                try:
                    ...
                    if RunningOS == 'win32':
                        ...
                        enc = 'utf-8'
                    tar.add(common.smart_unicode(picture).encode(
                        sys_encoding), common.smart_unicode(arcname).encode(enc))
                except:
                    common.log("show_pics >> zip",
                               "tar.gz compression error :", xbmc.LOGERROR)
                    error += 1
                    common.log("show_pics >> zip",  "Error  %s" % common.smart_unicode(
                        arcname).encode(sys_encoding), xbmc.LOGERROR)
                    print_exc()
        ...
        if self.args.viewmode == "export":
            ...
            dstpath = dialog.browse(3, common.getstring(
                30180), "files", "", True, False, "")
            dstpath = common.smart_unicode(dstpath)

            if dstpath == "":
                return

            # do you want to create a folder for exported pictures ?
            ok = dialog.yesno(common.getstring(30000), common.getstring(
                30181), "(%s)" % self.args.name)
            if ok:
                dirok = False
                while not dirok:
                    ...
                    if (kb.isConfirmed()):
                        ...
                        common.show_notification(common.getstring(
                            30000), common.getstring(30183), 3000, join(home, "icon.png"))
                        return
            ...
            for path, filename in filelist:
                ...
                pDialog.update(int(100*i/len(filelist)), common.getstring(30185) %
                               # "Copying '%s' to :"
                               join(path, filename), dstpath)
                i = i+1.0
                if isfile(join(dstpath, filename)):
                    ok = dialog.yesno(common.getstring(30000), common.getstring(
                        # File %s already exists in... overwrite ?
                        30186) % filename, dstpath, common.getstring(30187))
                    if not ok:
                        continue
            ...
            xbmc.sleep(1000)
            common.show_notification(common.getstring(30000), common.getstring(
                30189) % (cpt, dstpath), 3000, join(home, "icon.png"))
            # show the folder which contain pictures exported
            dialog.browse(2, common.getstring(30188),
                          "files", "", True, False, dstpath)
            return
        ...
        for path, filename in filelist:
            ...
            # 3 -
            context.append((common.getstring(30060), "RunPlugin(\"%s?action='locate'&filepath='%s'&viewmode='view'\" ,)" % (
